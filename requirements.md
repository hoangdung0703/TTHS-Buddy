...
- academic_reference LUÔN được query song song với legal_text (xem "BUG PHÁT HIỆN SAU" bên dưới - đã bỏ điều kiện gọi có chọn lọc ban đầu).
...
BUG PHÁT HIỆN SAU (không phải lúc build ban đầu, phát hiện qua 3 câu hỏi test thật về "phương pháp điều chỉnh"/"đối tượng điều chỉnh"/"lịch sử phát triển" của LTTHS) — cùng 1 gốc vấn đề: ranh giới giữa "legal_text được dùng làm căn cứ thật để trả lời" và "legal_text chỉ liên quan/trùng lặp nội dung" chưa được hệ thống phân biệt rõ, biểu hiện qua 3 lớp riêng biệt, sửa từng lớp:

1. Retrieval either/or (lớp Qdrant query): điều kiện needs_academic ban đầu là short-circuit - nếu legal_primary có điểm max ≥ threshold thì KHÔNG BAO GIỜ gọi academic, kể cả khi điểm legal cao chỉ do trùng từ khóa ngẫu nhiên (câu hỏi "phương pháp điều chỉnh của LTTHS" match nhầm "Điều 1. Phạm vi điều chỉnh" điểm 0.7+ dù nội dung không liên quan, trong khi chunk academic đúng có điểm 0.73-0.78 nhưng không bao giờ được query tới). Sửa: bỏ short-circuit, LUÔN query song song cả legal_text và academic_reference (asyncio.gather) rồi merge theo điểm, không loại trừ lẫn nhau - vì 2 nguồn trả lời 2 loại câu hỏi khác nhau, điểm cao ở 1 nguồn không có nghĩa nguồn kia không cần thiết. Rẻ hơn (chỉ thêm 1 query Qdrant mỗi lượt) và tổng quát hơn vá thêm từ khóa vào ANALYTICAL_INTENT_KEYWORDS (cùng loại lỗi heuristic đã từ bỏ ở Phase 3 title-truncation).

2. Citations liệt kê thừa (lớp API response): sau khi sửa (1), legal_primary vẫn có thể chứa Điều điểm cao do trùng từ khóa (Điều 1/7...) được retrieve vào context, nhưng model KHÔNG THỰC SỰ dùng để trả lời (academic mới là căn cứ chính) - citations vẫn liệt kê đủ vì _build_citations lấy nguyên legal_primary, không lọc theo việc model có thực sự cite hay không. Sửa: citations giờ lọc theo Điều model THỰC SỰ nhắc tới trong câu trả lời đã sinh ra (regex DIEU_NUMBER_PATTERN trên answer_text, dựa vào rule bắt buộc "Theo Điều [số]..."), không phải toàn bộ context đã retrieve; suggested_followups cũng đổi sang dựa trên Điều thực sự được cite đầu tiên. Do cần câu trả lời hoàn chỉnh mới lọc được, event SSE "citations" dời xuống sau "answer_delta" thay vì trước (đã xác nhận frontend không phụ thuộc thứ tự event).

3. Gán khung sai nguồn gốc khái niệm (lớp system prompt/văn phong): ngay cả khi Điều luật ĐƯỢC dùng thật (không phải trường hợp (2)) và nội dung có liên quan, model vẫn trình bày như thể Điều luật đó trực tiếp định nghĩa đúng thuật ngữ LÝ LUẬN được hỏi (ví dụ trình bày "QUY ĐỊNH PHÁP LUẬT - Theo Điều 1..." như thể Điều 1 "Phạm vi điều chỉnh" định nghĩa đúng khái niệm học thuật "đối tượng điều chỉnh", trong khi 2 khái niệm liên quan nhưng khác nhau về lý luận) - gây hiểu lầm nguồn gốc khái niệm cho người học. Sửa: thêm rule vào RAG_SYSTEM_PROMPT (rag_prompts.py) yêu cầu khi Điều luật không dùng đúng thuật ngữ lý luận trong tiêu đề/nội dung, phải diễn đạt là "có nội dung liên quan" thay vì ngầm coi là định nghĩa trực tiếp; định nghĩa/giải thích đúng thuật ngữ phải lấy từ TÀI LIỆU HỌC THUẬT. Đã cân nhắc hướng thay thế (không tách mục QUY ĐỊNH PHÁP LUẬT nếu thuật ngữ không khớp) nhưng loại vì đòi hỏi heuristic khớp chuỗi câu hỏi ↔ tiêu đề Điều luật - cùng loại lỗi heuristic đã từ bỏ ở Phase 3 title-truncation; sửa ở tầng prompt/model tự tuân thủ nhất quán với cách rule 2 (phân biệt nguồn) đã vận hành.

Kết quả evaluation sau cả 3 lớp sửa (backend/evaluation/run_evaluation.py, 29 câu, cộng thêm regression-check trực tiếp qua stream_answer_question do sandbox clock skew ~223s làm JWT bị 401 lúc chạy qua HTTP): groundedness 100%, correct-refusal 100%, academic-reference usage 100%, citation exact_match_rate 95% (1 fail còn lại là lỗi retrieval Nghị định 250/Điều 7 đã biết từ trước, không liên quan 3 lớp sửa này), mean_precision đi từ 41% (trước sửa (2)) → 78% (sau sửa (2)) → 74% (sau sửa (3), giảm nhẹ vì model giờ trích thêm Điều liên quan một cách trung thực hơn ở vài câu phân tích - không phải regression).
