...
[ ] Migration `backend/migrations/0008_chat_query_logs_rule9_grounding.sql` (thêm cột `rule9_ungrounded_dieu_numbers jsonb` vào `chat_query_logs` + index cho dữ liệu giám sát liên tục) - CHƯA chạy, cần chạy thủ công trong Supabase SQL editor (không có quyền DDL qua service-role key) trước khi wire field này vào `chat_log_service.py`.
[ ] Bước 3 — Build context có cấu trúc cho generation: mỗi sub_question + context riêng được đánh dấu rõ ràng (ví dụ "PHẦN 1: [câu hỏi con] — NGỮ CẢNH: [context riêng]"), yêu cầu model trả lời TỪNG PHẦN dùng ĐÚNG context được gán cho phần đó — tránh model "mượn" Điều luật của câu hỏi con này trả lời cho câu hỏi con khác. [ ] Bước 4 — Tận dụng fix Việc 1 (citations không bị xóa oan theo is_fallback toàn văn bản) — đã sẵn sàng hỗ trợ đúng kịch bản này: nếu 1 phần fallback (thiếu dữ liệu như case Chương XXVIII), các phần khác vẫn giữ nguyên citations đúng. [ ] Test bắt buộc: dùng đúng case UAT 6 câu hỏi (4 nhận định + 2 tình huống) làm test chính — so sánh citations/kết luận từng câu con TRƯỚC/SAU khi có tách câu hỏi, đối chiếu với Điều luật đúng đã xác định qua điều tra mục B (Điều 49/53, Điều 72 khoản 4b, Chương XXVIII đã bãi bỏ nên vẫn fallback đúng, Điều 174 khoản 2, Điều 157+359, Điều 357 khoản 2). [ ] Test không gây tác dụng phụ: câu hỏi đơn-chủ-đề bình thường (không có cấu trúc liệt kê) phải giữ nguyên hành vi cũ, không bị ép tách/chậm đi. [ ] Cân nhắc chi phí: mỗi sub_question thêm 1 lần gọi retrieve (rẻ, chỉ Qdrant + embedding), generation vẫn 1 lần duy nhất cho toàn bộ câu trả lời có cấu trúc (không tăng số lần gọi Gemini cho phần tốn kém nhất). [ ] Chạy lại 29 câu evaluation sau khi hoàn thành, xác nhận không hồi quy cho câu hỏi đơn-chủ-đề.