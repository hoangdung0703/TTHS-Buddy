...
[ ] A — LLM re-ranking: sau khi có HyDE (nếu B được giữ), lấy top-25 ứng viên từ semantic search, dùng 1 lời gọi Gemini nhẹ chấm điểm liên quan thật (đọc cùng lúc câu hỏi + từng đoạn ứng viên, không chỉ dựa cosine similarity), chọn lại legal_primary theo điểm re-rank thay vì thứ tự similarity thô. Test bằng đúng case Điều 165/110 (crowding do trùng từ vựng) — đây là case re-ranking được kỳ vọng giải quyết tốt nhất (phân biệt "liên quan thật" vs "trùng từ vựng bề mặt"). Đo mức cải thiện THÊM so với chỉ có B — để biết A có thực sự cần thiết hay B đã đủ. Chạy 29 câu eval, đo latency tăng thêm (re-ranking thêm 1 lời gọi Gemini mỗi request).

[ ] Quyết định cuối: nếu B đơn lẻ đã đạt hiệu quả tốt (không cần A), có thể dừng ở B để tiết kiệm latency/chi phí — không bắt buộc phải dùng cả A+B nếu A không mang lại cải thiện đáng kể.