...
[x] Query Understanding — 1 lượt Gemini nhẹ chạy TRƯỚC retrieval, làm 2 việc trong cùng 1 lần gọi:
    ...
    QUYẾT ĐỊNH (chốt sau khi test thật, không siết thêm ràng buộc): Query understanding được phép suy luận phạm vi văn bản mặc định (BLTTHS) khi câu hỏi không nêu rõ và có nhiều văn bản khả dĩ (ví dụ nhiều văn bản đã ingest đều có "Điều 15" riêng), dựa trên domain của sản phẩm — đây là suy luận phạm vi (scope), không phải bịa nội dung pháp lý. Nếu người dùng cần tuyệt đối tránh giả định này, có thể tự nêu rõ tên văn bản trong câu hỏi. Test thật xác nhận hành vi này giữ đúng grounding: câu hỏi tiếp nối "còn áp dụng thế nào khi bị can không hợp tác?" sau khi đã hỏi Điều 15 được viết lại đúng ngữ cảnh, và khi dữ liệu Điều 15 không có nội dung đó, hệ thống trả lời fallback đúng thay vì bịa.
